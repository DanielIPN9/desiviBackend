...
public class GenericReportTest {
	...
	@Test
	@DisplayName("VALID INVENTORY ACCOUNTING REPORT")
	public void validInventoryAccountingReport() {
		log.info("INIT validInventoryAccountingReport()");
		...
		ResponseModel responseReport = reportServicePort.generateAccountingInventoryReport(inventoryParamsReport);
		...
		try {
			...
			FileUtils.writeByteArrayToFile(
					new File("C:\\Users\\Odalis\\Documents\\reports\\accounting_inventory_report_" + date + format),
	...
	public void validInventoryReport() {
		...
		try {
			...
			String format = inventoryParamsReport.getFormat().equals("PDF") ? ".pdf" : ".xls";
			FileUtils.writeByteArrayToFile(
					new File("C:\\Users\\Odalis\\Documents\\reports\\inventory_report_" + date + format),
					(byte[]) responseReport.getData());
		} catch (IOException e) {
			log.severe(e.getMessage());
		}
